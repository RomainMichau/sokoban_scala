...
object PushBoxNode {
  def apply(state: PushBoxNodeState, initialPos: Coord): PushBoxNode =
    new PushBoxNode(state, None, Some(initialPos))
}
...
/**
  ...
  */
case class PushBoxNode private (
    state: PushBoxNodeState,
    incomingEdge: Option[PushBoxEdge],
    initialPos: Option[Coord] = None
) extends BFSNode[PushBoxNode] {

  if (initialPos.isDefined && incomingEdge.isDefined) {
    throw new Exception(
      "initialPos and incomingEdge are both defined. Only one of them must be defined"
    )
  }

  lazy val currentPos: Coord = incomingEdge
    .map(_.incommingMove.arrivalCoord)
    .getOrElse(
      initialPos.getOrElse(throw new Exception("no inital coord defined"))
    )
  ...
  override def getConnectedNode: Set[PushBoxNode] = {
    this.state.accessibleZone.potentialPushBoxes.flatMap { move =>
      ...
      if (
        field(newBoxPos).isWalkable && field(
          newBoxPos
        ).sqType != SquareType.Deadlock
      ) {
        val newField = field.pushBox(boxPos, move.direction)
        if (!isADynamicDeadLock(newBoxPos)) {
          Some(
            PushBoxNode(
              PushBoxNodeState(newField, AccessibleZone(newField, boxPos)),
              Some(PushBoxEdge(this, move))
            )
          )
        } else {
          None
  ...
  private def isADynamicDeadLock(newBox: Coord): Boolean = {
    ...
    def isBoxBlocked(box: Coord): Boolean = {
      ...
      val boxDirectlyPushable = Direction.values.exists { dir =>
        ...
        state.accessibleZone.accessibleCoord(behindCoord) && field(
          frontCoord
        ).isWalkable && field(behindCoord).isWalkable
      }
      val adjacentBoxIsPushable = box.adjacentSq
        .filter { adjCoord =>
          adjCoord.isInField &&
    ...
    val res =
      field(newBox).sqType != SquareType.BoxPlaced && isBoxBlocked(newBox)
    /* if(res) {
      field.drawField()
      print(res)
    }*/
  ...
  def toDirs: immutable.Vector[Direction] = {
    (this.getPathToNode.drop(1) :+ this).flatMap { node =>
      SolverHelper
        .getPathAsAPlayerCannotPushBox(
          node.parentNode.get.currentPos,
          node.incomingEdge.get.incommingMove.initialCoord,
          node.parentNode.get.field
        )
  ...
  override def draw(): Unit =
    field.drawField(state.accessibleZone.accessibleCoord.headOption)

  override def toString: String =
    this.incomingEdge
      .map(edge =>
        edge.incommingMove.initialCoord.toString + edge.incommingMove.direction
      )
      .getOrElse("First Node")

  def details: String =
    s"boxes: ${field.getBoxes}, nPos ${state.accessibleZone.nPos}"
}

case class PushBoxNodeState(field: Field, accessibleZone: AccessibleZone) {
  def this(gameState: GameState) =
    this(gameState.field, AccessibleZone(gameState.field, gameState.playerPos))
...
case class PushBoxEdge(parentNode: PushBoxNode, incommingMove: Move)
    extends BFSIncomingEdge[PushBoxNode]
