...
object SolverHelper {
  ...
  def getDistMap(field: Field): Map[Coord, Int] = {
    val map = field.getAllSquares
      .filter(_.sqType != SquareType.Wall)
      .map { sq =>
        val node: MoveNode = MoveNode.getMoveNodeAsABoxInAnEmptyField(
          MoveNodeState(field, sq.coord)
        )
        (sq.coord -> BFS
          .doBFS(
            node,
  ...
  def getPathAsAPlayerCannotPushBox(
      initCoord: Coord,
      arrivalCoord: Coord,
      field: Field
  ): Option[Vector[Move]] = {
    val node: MoveNode = MoveNode.getMoveNodeAsAPlayerWhoCantPushBox(
      MoveNodeState(field, initCoord)
    )
